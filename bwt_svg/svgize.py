...
def render(t,
    ...
    assert which in ["horizontal", "vertical", "both"]
    ...
    if which in ["vertical", "both"]:
        from_right = 0
        bwm_start_y = (10 * ch_ht + padding - 30) if which == 'both' else (3 * ch_ht)
...
def main():
    ...
    render_parser.add_argument('--output', '-o', default='bwt_diagram.svg',
                              help='Output SVG for combined diagram (default: combined.svg)')
    render_parser.add_argument('--output-top', default=None,
                              help='Output SVG filename for top portion of drawing (default: not output)')
    render_parser.add_argument('--output-bottom', default=None,
                              help='Output SVG filename for bottom portion of drawing (default: not output)')
    ...
    if args.command == 'print':
        ...
        if args.output_top:
            svg_content = render(args.text,
                ...
                show_thresholds=args.show_thresholds,
                guidelines=args.show_guidelines)
            with open(args.output_top, 'w', encoding='utf-8') as f:
                f.write(svg_content)
            print(f"Top portion of SVG saved to {args.output_top}")

        if args.output_bottom:
            svg_content = render(args.text,
                which="vertical",
                background_color=args.background_color,
                monospace_font=args.monospace_font,
                ...
                show_thresholds=args.show_thresholds,
                guidelines=args.show_guidelines)
            with open(args.output_bottom, 'w', encoding='utf-8') as f:
                f.write(svg_content)
            print(f"Bottom portion of SVG saved to {args.output_top}")
    else:
        parser.print_help()
